...
\begin{figure}[h]
    ...
    \caption{Segmentation predictions by DeepLabV3 (frozen) on the Cityscapes validation set. Top: a successful prediction; input (left) and predicted mask (right) show accurate road, vegetation, and building segmentation. Bottom: a failure case; input (left), ground truth (center), and prediction (right) reveal that pedestrians and traffic signs are missed at $256\times256$ resolution.}
...
The qualitative predictions in Fig.~\ref{fig:predictions} illustrate two characteristic patterns. Spatially dominant, texture-rich classes such as road, building, and vegetation are segmented reliably, consistent with the texture-bias account of ImageNet-pretrained CNNs~\cite{geirhos2019}. Small-object classes such as pedestrians and traffic signs are poorly segmented at $256\times256$ resolution: fine-grained spatial detail is lost by the downsampling and cannot be recovered by augmentation. These categories also carry disproportionately low weight in the aggregate Mean Dice metric, which is dominated by the large Flat and Construction classes.
...
\textbf{Limitations.}
The reported Dice scores (0.46--0.47) are substantially below state-of-the-art results ($>$0.80 Mean IoU); this reflects the experimental constraints of $256\times256$ downsampling and training only the classifier head, and all comparisons should be interpreted within this constrained setting. Several additional constraints limit the strength of conclusions. The corrupted test set returns only an aggregate score, so it is not possible to verify whether the gains extend to structural corruptions such as blur, which color jitter does not directly simulate. To bound training variance, multi-seed evaluation was conducted on the validation set (U-Net: $n{=}3$, seeds 42/0/1; DeepLabV3 frozen: $n{=}2$, seeds 0/1), yielding standard deviations of $\pm0.010$ and $\pm0.002$ Mean Dice respectively. This indicates that the reported differences are not attributable to a favourable random initialisation, though the small clean-data gain ($+0.0083$ Mean Dice on the test set) remains modest relative to the variance bound. The U-Net and DeepLabV3 configurations also differ in learning rate scheduling (fixed vs.\ cosine annealing), an uncontrolled variable that partially conflates with the architecture comparison.

\textbf{Future work.}
Future directions include per-corruption-type evaluation to directly test \textbf{H2}; higher input resolutions to improve small-object segmentation; and Cityscapes-specific improvements such as class-weighted loss or weather-conditioned augmentation.
